...
namespace C969Assessment
{
    ...
    public class Appointment
    {
        ...
        public static List<Appointment> getCurrentUserAppts()
        {
        ...
        public static void appointmentReminder()
        {
            foreach (Appointment appt in apptList)
            {
                if (appt.userId == userContext.getUserId())
                {
                    if (DateTime.Now.Subtract(DateTime.Parse(appt.start)).TotalMinutes <= 15 && DateTime.Now.Subtract(DateTime.Parse(appt.start)).TotalMinutes > 0)
                    {
