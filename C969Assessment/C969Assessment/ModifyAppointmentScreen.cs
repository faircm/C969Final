...
namespace C969Assessment
{
    public partial class ModifyAppointmentScreen : Form
    {
        public ModifyAppointmentScreen(DataGridViewRow appt)
        {
            ...
            apptIdBox.Text = selectedAppt.appointmentId.ToString();
        ...
        private void submitBtn_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    if (appt.appointmentId == Int32.Parse(apptIdBox.Text))
                    {
                        continue;
                    }
                    else
                ...
                Appointment newAppt = new Appointment(Int32.Parse(apptIdBox.Text), Int32.Parse(custIdBox.Text), Int32.Parse(userIdBox.Text), titleBox.Text, descBox.Text, locationBox.Text, contactBox.Text, typeBox.Text, urlBox.Text, startTimePicker.Value.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"), endTimePicker.Value.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"), createDatePicker.Value.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"), createdByBox.Text, endTimePicker.Value.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss"), lastUpdateByBox.Text);
